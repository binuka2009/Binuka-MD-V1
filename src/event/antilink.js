...
export const handleAntilink = async (m, sock, logger, isBotAdmins, isAdmins, isCreator) => {
    ...
    if (cmd === 'antilink') {
        ...
        if (!m.isGroup) {
            await sock.sendMessage(m.from, { text: '*This command can only be used in groups...⚙️*' }, { quoted: m });
            return;
        }
    ...
    if (antilinkSettings[m.from]) {
        if (m.body.match(/(chat.whatsapp.com\/)/gi)) {
            if (!isBotAdmins) {
                await sock.sendMessage(m.from, { text: `*Binuka-MD bot needs to be an admin to remove links...⛔*` });
                return;
            }
            ...
            await sock.sendMessage(m.from, {
                text: `\`\`\`「 Group Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]}, Please Don't Share Link In This Group...🌟`,
